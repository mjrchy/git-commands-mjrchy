...
3. (Pushing a new branch) You commit some files to the `dev-foo` branch and try to "push" them to Github, but it fails as shown here:
   ...
   ```
   Explain this error.
   >  the branch you are trying to push isn't push to the local repository yet so you need to type git push -u dev-foo first to push your dev-foo branch to the local repository.
...
6. Consider this situation:
   ...
   When I push my change, it will cause an error because the file in a local repository and working copy are different so you need to fix this error by using this git command  and incorporate the changes from the remote repository before pushing your local changes. 
   ```
   git fetch origin
   ```   
